...
def get_gemini_model():
    ...
    try:
        ...
        print(f"Gemini configured failed: {e}", file=sys.stderr)
        return None
...
def chat():
    ...
    try:
        ...
        txn_res = sb.table("transactions").select("date,merchant,category,amount,status").eq("user_id", user_id).order("date", desc=True).limit(1000).execute()
        
        context_parts = ["You are a Financial Risk Assistant helping the user monitor their portfolio within the Financial Risk Engine platform."]
