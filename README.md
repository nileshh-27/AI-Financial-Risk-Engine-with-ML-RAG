...
- **Instant Insights:** Receive a sleek HTML report via email immediately after PDF analysis is complete.
...
- **Backend:** Node Express Proxy, Flask (Python 3.10+), Pandas, Scikit-learn, PDFPlumber.
...
The platform is deployed using a decoupled, high-performance architecture:
- **Frontend (Static UI):** Deployed on **Netlify** for edge-accelerated asset delivery.
- **Express API Proxy:** Deployed on **Render** (Node.js) to securely route client traffic.
- **Python ML Backend:** Deployed on **Render** (Flask + Gunicorn) to handle intensive PDF parsing and LLM orchestration.

## ⚙️ Installation & Setup

1. **Clone the repository**
   ```bash
   git clone https://github.com/nileshh-27/AI-Financial-Risk-Engine-with-ML-RAG.git
   cd AI-Financial-Risk-Engine-with-ML-RAG
   ```
...
5. **Run the Application Locally**
   - **Start the Python Backend (runs on port 5002):** 
     ```bash
     cd backend && python app.py
     ```
   - **Start the Express Server & Vite Frontend (runs on port 5173 / 5001):** 
     ```bash
     npm run dev
     ```
